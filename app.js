...
function renderPracticeMode(bankId,newOnly){
  ...
  $("#startPractice").onclick=()=>{
    ...
    const s=newPractice(bankId,mode,newOnly);if(!s.questionIds.length){alert("Không có câu phù hợp.");return;}deleteSession(key);saveSession(s);activateSession(s);
  };
}

function renderExamSetup(selectedGroup="specialist",groupCode="A",bankId=null){
  CURRENT_DATA=ACTIVE_DATA;const bp=BLUEPRINTS[groupCode];const specialists=specialistManifest().filter(x=>bp.allowed.includes(x.id));
  if(!bankId||!specialists.some(x=>x.id===bankId))bankId=specialists[0]?.id;
  const detail=selectedGroup==="specialist"?specialists.map(x=>`<button class="btn3d choice-card ${x.id===bankId?"selected":""}" data-exam-bank="${x.id}"><div class="strong">${esc(x.name)}</div><div class="small mt8">75 câu chuyên môn trong đề</div></button>`).join(""):
    `<div class="notice info"><b>Kiến thức chung / Phụ trợ được hệ thống tự ghép theo đối tượng thi.</b><div class="mt8">${Object.entries(bp.general).map(([cat,n])=>`${n} ${CATEGORY_NAMES[cat]}`).join(" · ")}</div></div>`;
  ...
  ${groupButtons(selectedGroup)}
  <section class="panel mt12"><div class="section-title">${selectedGroup==="specialist"?"Chọn chuyên đề / nghiệp vụ thi":"Cơ cấu kiến thức chung / phụ trợ"}</div><div class="choice-list">${detail}</div></section>
  <section class="panel"><div class="section-title">Đối tượng thi</div><div class="choice-list">${Object.entries(BLUEPRINTS).map(([k,v])=>`<button class="btn3d choice-card ${k===groupCode?"selected":""}" data-candidate="${k}">${esc(v.name)}</button>`).join("")}</div></section>
  <section class="panel soft center"><div class="eyebrow">ĐỀ SẼ TẠO</div><div class="strong mt8">ĐỀ THI NGHIỆP VỤ<br>${esc((bank(bankId)?.meta.name||"").toUpperCase())}</div><div class="small strong mt8">${esc(bp.name)}</div><div class="small mt12"><b>100</b> câu · <b>45</b> phút</div></section>
  <button class="btn3d primary" style="width:100%" id="startExam" ${!bankId?"disabled":""}>Tạo đề & bắt đầu thi</button><div class="center tiny mt8">Thời gian bắt đầu tính ngay khi đề được tạo.</div>`;
  $("#backHome").onclick=renderHome;document.querySelectorAll("[data-group]").forEach(b=>b.onclick=()=>renderExamSetup(b.dataset.group,groupCode,bankId));
  document.querySelectorAll("[data-candidate]").forEach(b=>b.onclick=()=>renderExamSetup(selectedGroup,b.dataset.candidate,null));
  document.querySelectorAll("[data-exam-bank]").forEach(b=>b.onclick=()=>renderExamSetup(selectedGroup,groupCode,b.dataset.examBank));
  $("#startExam").onclick=()=>{
    const key=storageKeyExam(groupCode,bankId),existing=loadSession(key);if(existing&&!confirm("Đang có bài thi đã lưu. Tạo đề mới sẽ thay thế bài cũ. Tiếp tục?"))return;
    try{deleteSession(key);const s=newExam(groupCode,bankId);saveSession(s);activateSession(s);}catch(e){alert(e.message);}
  };
}
...
function renderSession(){
  ...
  if(s.kind==="practice"&&locked){
    ...
    else{const chosen=q.options.find(o=>o.id===state.selectedOptionId),correct=q.options.find(o=>o.correct);feedback=`<div class="feedback bad">❌ Không chính xác.<div class="small mt8">Bạn chọn: ${esc(chosen?.text||"")}<br>Đáp án đúng: ${esc(correct?.text||"")}</div></div>`;}
  }
  const newBadge=q.releaseStatus==="NEW"?`<span class="badge new">🆕 Mới</span>`:"";
  app().innerHTML=`<section class="panel">${questionTitle(s)}<div class="question-head mt12"><span class="badge primary">Câu ${s.index+1}</span>${newBadge}</div><div class="question">${esc(q.question)}</div>
  ...
  ${s.kind==="practice"?`<div class="toolbar6"><button class="btn3d" id="prev">Câu trước</button><button class="btn3d primary" id="grade">Đáp án</button><button class="btn3d" id="next">Câu tiếp theo</button><button class="btn3d" id="flag">${state.flagged?"Bỏ đánh dấu":"Đánh dấu"}</button><button class="btn3d" id="skip">Bỏ qua</button><button class="btn3d" id="stop">Dừng/Kết thúc</button></div>`:
  `<div class="toolbar3"><button class="btn3d" id="prev">Câu trước</button><button class="btn3d" id="flag">${state.flagged?"Bỏ đánh dấu":"Đánh dấu"}</button><button class="btn3d" id="next">Câu tiếp theo</button></div><button class="btn3d danger mt8" style="width:100%" id="stop">Dừng / Nộp bài</button>`}
  ...
  document.querySelectorAll("[data-jump]").forEach(b=>b.onclick=()=>{s.index=Number(b.dataset.jump);saveSession(s);renderSession();});
  $("#prev").disabled=s.index===0;$("#prev").onclick=()=>{s.index--;saveSession(s);renderSession();};
  $("#next").disabled=s.index>=s.questionIds.length-1;$("#next").onclick=()=>{s.index++;saveSession(s);renderSession();};
  $("#flag").disabled=locked;$("#flag").onclick=()=>{state.flagged=!state.flagged;saveSession(s);renderSession();};
  if(s.kind==="practice"){
    $("#grade").disabled=locked||!state.selectedOptionId;$("#grade").onclick=()=>{const correct=q.options.find(o=>o.correct);state.correctOptionId=correct.id;state.isCorrect=state.selectedOptionId===correct.id;state.locked=true;state.flagged=false;saveSession(s);renderSession();};
    $("#skip").disabled=locked;$("#skip").onclick=()=>{state.selectedOptionId=null;saveSession(s);if(s.index<s.questionIds.length-1)s.index++;saveSession(s);renderSession();};
...
function showPracticeStop(){
  ...
  $("#goWrong").disabled=c.wrong===0;$("#goWrong").onclick=()=>{const i=nextIndexMatching(qid=>practiceStatus(s,qid)==="wrong");if(i!==null)s.index=i;closeModal();renderSession();};

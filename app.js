...
const APP_VERSION = "Mobile V1.1.3";
...
function renderSession(){
  ...
  if(s.kind==="practice"&&s.reviewOnly==="wrong"){
    ...
    $("#prev").onclick=()=>{if(prevWrong!==null){s.index=prevWrong;saveSession(s);renderSession();}};
    $("#next").disabled=nextWrong===null;
    $("#next").onclick=()=>{if(nextWrong!==null){s.index=nextWrong;saveSession(s);renderSession();}};
  }else{
...
function showPracticeStop(){
  ...
  $("#goWrong").disabled=c.wrong===0;$("#goWrong").onclick=()=>{s.reviewOnly="wrong";const i=nextIndexMatching(qid=>practiceStatus(s,qid)==="wrong");if(i!==null)s.index=i;closeModal();renderSession();};
